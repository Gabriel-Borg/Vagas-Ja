...
window.initMap = function() {
    ...
    // document.getElementById('location-input').addEventListener('keypress', searchNearby); 
 ...
 function searchNearby() {
    ...
    service.nearbySearch(request, function (results, status) {
        if (status === google.maps.places.PlacesServiceStatus.OK) {
            clearMarkers();
            exibirEstacionamentos(results); // Passa os resultados para a função exibirEstacionamentos
...
function exibirEstacionamentos(estacionamentos) {
    ...
    estacionamentos.forEach(estacionamento => {
        ...
        const fotoUrl = estacionamento.photos && estacionamento.photos.length > 0 
            ...
            : 'image/placeholder.png'; // Usa uma imagem placeholder se não houver foto
        ...
        itemEstacionamento.innerHTML = `
            ...
            <p><strong>Vagas Disponíveis:</strong> Não informado</p>
